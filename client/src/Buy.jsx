...
function BuyProperties() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/api/properties") // change to your deployed backend URL
      .then((res) => {
        // Filter only 'buy' category properties (case-insensitive)
        const buyProperties = res.data.filter(
          (p) => p.category && p.category.toLowerCase() === "buy"
        );
  ...
  return (
    <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
      {properties.length === 0 && <p>No properties found for category Buy.</p>}
